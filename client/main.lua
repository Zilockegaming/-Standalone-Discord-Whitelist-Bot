...
function lockpickDone(success)
    ...
    if success then
        ...
        if math.random(1, 100) <= 40 and IsWearingHandshoes() then
            TriggerServerEvent("evidence:server:CreateFingerDrop", pos)
            QBCore.Functions.Notify("Je handschoenen zijn gescheurd..")
        end
...
function PoliceAlertMessage(dispatch)
    if dispatch == "robbery" then
        exports['dispatch']:IFruitStoreRobbery()
        copsCalled = true
    elseif dispatch == "poweroutage" then
        exports['dispatch']:PowerOutage()
    elseif dispatch == "powerboxexplosion" then
        exports['dispatch']:PowerBoxExplosion()
    else
...
function ThermiteSuccess()
    QBCore.Functions.Notify("Je hebt met succes het beveiligingssysteem gehackt!", "success")
    ...
    if usingSafe then
        if won then
            if not Config.Locations["safe"].isDone then
                ...
                TriggerServerEvent("qb-ifruitstore:server:SafeReward")
                TriggerServerEvent("qb-ifruitstore:server:SetSafeStatus", "isBusy", false)
                TriggerServerEvent("qb-ifruitstore:server:SetSafeStatus", "isDone", true)
                takeAnim()
            end
        end
    end
                    ...
                    if not isCooldown then
                        QBCore.Functions.TriggerCallback("QBCore:HasItem", function(hasItem)
                            if hasItem then
                                ...
                                QBCore.Functions.Notify("Je beschikt niet over de juiste uitrusting!", "error")
                            end
                        end, "thermite")
                    else
                        QBCore.Functions.Notify("Dit is zojuist bereikt, je zult moeten wachten!", "error")
                    end
                end)
            else
                QBCore.Functions.Notify("In ieder geval niet genoeg politie online (".. Config.RequiredCops ..") zijn er minimaal nodig!", "error")
            end
        end
    end
end)
                                ...
                                if CurrentCops >= Config.RequiredCops then
                                    QBCore.Functions.TriggerCallback("QBCore:HasItem", function (hasItem)
                                        if hasItem then
                                            ...
                                            QBCore.Functions.Notify("Je mist een geavanceerde lockpick", "error")
                                        end
